  ...
  return (
    <div className={twMerge('relative inline-block')}>
      ...
      <button
        ...
        className={twMerge(
          'flex items-center text-sm justify-between  w-[220px] pl-3 pr-2 py-[10px] rounded-lg border-2 border-border-element dark:border-border-element-dark bg-bg-primary dark:bg-bg-primary-dark shadow-xs transition',
          className
        )}
      >
      ...
      {/* Calendar Popup */}
      {open && (
        <div className="absolute mt-2 w-64 bg-white border rounded-xl shadow-lg z-50 p-3">
          {/* Header month */}
          ...
          {/* Days of week */}
          <div className="grid grid-cols-7 text-center text-text-lo text-sm mb-1">
              ...
              return (
                <button
                  ...
                  className={`aspect-square w-full rounded-full text-sm flex items-center justify-center transition ${
                    isSelected ? 'bg-blue text-white' : isCurrentMonth ? 'hover:bg-blue-100 text-text-hi' : 'text-text-lo'
                  }`}
                >
                  {day.date()}
                </button>
