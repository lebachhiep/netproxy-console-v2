...
export const SelectTag: React.FC<SelectTagProps> = ({ options, value, defaultValue, placeholder = 'Chọn', className, onChange }) => {
  ...
  return (
    <div ref={ref} className={twMerge('relative w-56')}>
      ...
      {/* Dropdown */}
      {open && (
        <div className="p-1 absolute top-full mt-1 w-full bg-bg-secondary dark:bg-bg-secondary-dark border border-border-element dark:border-border-element-dark rounded-lg shadow-md z-10">
          {options.map((opt) => (
            <div
              key={opt.value}
